...
module Test
  module BioPortal 
    ...
    if false
      ...
      800.times do |i|
        ont = Ontology.where(acronym: "ontology #{Random.rand(3)}").all.first
        owner = User.where(username: "user#{i % 200}name").include(:username).all.first
        n = Note.new(content: "content " * 60, owner: owner, ontology: ont)
        n.save
        puts "created note #{i}"
...
eos
        ...
        bench_result << [Time.now - start,count_sol,notes.length, client.query_times.last, client.parse_times.last ]
      end
      bench_result.select! { |x| x[2] > 0 }
      bench_result.sort_by! { |x| x[0] }
      CSV.open("benchmark_naive.csv", "wb") do |csv|
        csv << ["total", "sols", "notes", "qt", "pt"]
    ...
    def self.benchmark_query_goo
      ...
      ont.each do |ont|
        ...
        bench_result << [Time.now - start, notes.length,num_queries,agg_queries,agg_parsing ]
      end
      bench_result.select! { |x| x[1] > 0 }
      bench_result.sort_by! { |x| x[0] }
      CSV.open("benchmark_goo.csv", "wb") do |csv|
        csv << ["total", "notes", "queries", "agg_qt", "agg_qp"]
        bench_result.each do |b|
          csv <<  b
        end
      end
