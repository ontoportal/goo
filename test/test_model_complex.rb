...
class Term < Goo::Base::Resource
  ...
  attribute :parents, namespace: :rdfs, property: :subClassOf, enforce: [:list]
...
class TestModelComplex < MiniTest::Unit::TestCase
  ...
  def test_parents_inverse_children
    ...
    van.resource_id = RDF::URI.new "http://someiri.org/van"
    van.prefLabel = "van"
    ...
    cargo.resource_id = RDF::URI.new "http://someiri.org/cargo"
    ...
    minivan.resource_id = RDF::URI.new "http://someiri.org/minivan"
    ...
    cargovan.resource_id = RDF::URI.new "http://someiri.org/cargovan"
    ...
    vehicle = Term.find(RDF::URI.new("http://someiri.org/vehicle"), submission: submission)
    ch = vehicle.children
    assert ch.length == 2
